  ...
  <div class="space-y-6">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
      <div v-if="props.showTitle" class="space-y-1">
        ...
        <p class="text-xs text-brand-500">导入后的记录会长期保留，可按需筛选、禁用、重置或查看执行结果。</p>
      </div>
    ...
    <div class="flex flex-wrap gap-1 rounded-md bg-brand-100 p-0.5">
      <button v-for="tab in tabLabels" :key="tab.key" type="button" :class="['rounded-md px-3 py-2 text-sm transition', store.activeTab === tab.key ? 'bg-white font-medium text-brand-900 shadow-sm' : 'text-brand-500 hover:text-brand-900']" @click="store.handleTabChange(tab.key)">
        {{ tab.label }}
      </button>
    </div>

    <div v-if="store.isTaskListTab" class="grid gap-3 rounded-md border border-brand-300/50 bg-white p-4 shadow-sm lg:grid-cols-5">
    ...
    <div v-else-if="store.activeTab === 'resultList'" class="grid gap-3 rounded-md border border-brand-300/50 bg-white p-4 shadow-sm lg:grid-cols-6">
    ...
    <div v-else class="grid gap-3 rounded-md border border-brand-300/50 bg-white p-4 shadow-sm lg:grid-cols-4">
    ...
    <div class="flex flex-col gap-3 rounded-md border border-brand-300/50 bg-white px-4 py-3 shadow-sm sm:flex-row sm:items-center sm:justify-between">
      <span class="text-xs text-brand-500">第 {{ store.currentPage }} / {{ store.totalPages }} 页，共 {{ store.currentTotal }} 条</span>
