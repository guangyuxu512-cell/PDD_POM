  ...
  <div class="space-y-6">
    ...
    <section class="rounded-md border border-brand-200/50 bg-white p-5 shadow-sm">
      <div class="grid gap-4 lg:grid-cols-[320px_1fr] lg:items-end">
        <div class="space-y-2">
          ...
          <Listbox v-model="selectedShopId">
            <div class="relative">
              ...
              <ListboxOptions class="absolute z-20 mt-2 max-h-72 w-full overflow-auto rounded-md border border-brand-200/50 bg-white py-1 shadow-lg">
        ...
        <div class="rounded-md border border-brand-200/50 bg-gray-50 px-4 py-3 text-sm text-gray-500">
    ...
    <div v-if="!shops.length && !loading" class="rounded-md border border-brand-200/50 bg-white px-6 py-12 text-center text-sm text-gray-400 shadow-sm">🏪 暂无店铺，请先创建店铺</div>

    <section v-else class="space-y-4 rounded-md border border-brand-200/50 bg-white p-5 shadow-sm">
      <section class="space-y-4">
        <div class="space-y-1">
          <h2 class="text-sm font-medium text-gray-900">全局设置</h2>
          <p class="text-xs text-gray-500">启用自动售后与默认排除类型统一在此维护。</p>
        </div>
        <label class="flex items-center justify-between rounded-md border border-brand-200/50 bg-gray-50 px-4 py-3">
        ...
        <div class="space-y-3">
          ...
          <div class="flex flex-wrap gap-2">
            <button v-for="option in unsupportedTypeOptions" :key="option" type="button" class="rounded-full px-3 py-1 text-xs font-medium" :class="form.unsupportedTypes.includes(option) ? 'bg-brand-900 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'" @click="toggleSelection(form.unsupportedTypes, option)">{{ option }}</button>
          </div>
      ...
      <section class="space-y-4 border-t border-gray-100 pt-4">
        ...
        <div class="whitelist overflow-x-auto rounded-md border border-brand-200/50">
        ...
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <label class="space-y-2"><span class="text-xs font-medium text-gray-600">自动退款金额上限（元）</span><input v-model.number="form.autoRefundLimit" type="number" min="0" step="0.5" class="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500" /></label>
        </div>
        <label class="flex items-center justify-between rounded-md border border-brand-200/50 bg-gray-50 px-4 py-3"><span class="text-sm text-gray-700">需要入库校验</span><input v-model="form.requireInventoryCheck" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-gray-900 focus:ring-brand-500" /></label>
      </section>
      ...
      <section class="grid gap-4 border-t border-gray-100 pt-4 xl:grid-cols-2">
        <div class="space-y-4">
          ...
          <label class="flex items-center justify-between rounded-md border border-brand-200/50 bg-gray-50 px-4 py-3"><span class="text-sm text-gray-700">启用自动处理</span><input v-model="form.refundOnlyEnabled" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-gray-900 focus:ring-brand-500" /></label>
          <div class="grid gap-4 md:grid-cols-3">
            <label class="space-y-2"><span class="text-xs font-medium text-gray-600">自动同意金额上限（元）</span><input v-model.number="form.refundOnlyAutoApproveLimit" type="number" min="0" step="0.5" class="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500" /></label>
            <label class="space-y-2"><span class="text-xs font-medium text-gray-600">最大拒绝次数</span><input v-model.number="form.refundOnlyMaxRejectCount" type="number" min="0" class="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500" /></label>
            <label class="space-y-2"><span class="text-xs font-medium text-gray-600">拒绝后等待（分钟）</span><input v-model.number="form.refundOnlyRejectWaitMinutes" type="number" min="0" class="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500" /></label>
          </div>
          <div class="grid gap-3 sm:grid-cols-2">
            <label class="flex items-center justify-between rounded-md border border-brand-200/50 bg-gray-50 px-4 py-3"><span class="text-sm text-gray-700">启用拒绝策略</span><input v-model="form.refundOnlyNeedReject" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-gray-900 focus:ring-brand-500" /></label>
            <label class="flex items-center justify-between rounded-md border border-brand-200/50 bg-gray-50 px-4 py-3"><span class="text-sm text-gray-700">有图片转人工</span><input v-model="form.refundOnlyManualOnImages" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-gray-900 focus:ring-brand-500" /></label>
            <label class="flex items-center justify-between rounded-md border border-brand-200/50 bg-gray-50 px-4 py-3 sm:col-span-2"><span class="text-sm text-gray-700">拒收退回自动同意</span><input v-model="form.refundOnlyAutoApproveRejectedReturn" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-gray-900 focus:ring-brand-500" /></label>
          </div>
        </div>
        ...
        <div class="space-y-4">
          ...
          <div class="grid gap-3 sm:grid-cols-2">
            <label class="flex items-center justify-between rounded-md border border-brand-200/50 bg-gray-50 px-4 py-3"><span class="text-sm text-gray-700">飞书通知</span><input v-model="form.feishuNotifyEnabled" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-gray-900 focus:ring-brand-500" /></label>
            <label class="flex items-center justify-between rounded-md border border-brand-200/50 bg-gray-50 px-4 py-3"><span class="text-sm text-gray-700">微信通知</span><input v-model="form.wechatNotifyEnabled" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-gray-900 focus:ring-brand-500" /></label>
          </div>
          <label class="space-y-2"><span class="text-xs font-medium text-gray-600">飞书 Webhook</span><input v-model="form.feishuNotifyWebhook" type="text" placeholder="https://open.feishu.cn/..." class="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 placeholder:text-gray-400 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500" /></label>
          <label class="space-y-2"><span class="text-xs font-medium text-gray-600">微信群ID</span><input v-model="form.wechatGroupId" type="text" placeholder="请输入微信群ID" class="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 placeholder:text-gray-400 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500" /></label>
          <div class="space-y-3"><span class="text-xs font-medium text-gray-600">通知场景</span><div class="flex flex-wrap gap-2"><button v-for="scene in notifySceneOptions" :key="scene" type="button" class="rounded-full px-3 py-1 text-xs font-medium" :class="form.notifyScenes.includes(scene) ? 'bg-brand-900 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'" @click="toggleSelection(form.notifyScenes, scene)">{{ scene }}</button></div></div>
        </div>
      </section>
      <section class="grid gap-4 border-t border-gray-100 pt-4 xl:grid-cols-2">
        <div class="space-y-4">
          ...
          <label class="space-y-2"><span class="text-xs font-medium text-gray-600">弹窗下拉偏好</span><input v-model="form.popupSelectPreference" type="text" placeholder="例如：顺丰" class="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 placeholder:text-gray-400 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500" /></label>
          <div class="space-y-3">
            <span class="text-xs font-medium text-gray-600">弹窗选项偏好</span>
            <div class="flex gap-2"><input v-model="form.popupOptionInput" type="text" placeholder="输入后回车添加" class="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 placeholder:text-gray-400 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500" @keydown.enter.prevent="addPopupOptionPreference" /><button type="button" class="rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-700 hover:bg-gray-50" @click="addPopupOptionPreference">添加</button></div>
          ...
          <label class="space-y-2"><span class="text-xs font-medium text-gray-600">备注模板：退货匹配</span><input v-model="form.remarkReturnMatch" type="text" class="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500" /></label>
          <label class="space-y-2"><span class="text-xs font-medium text-gray-600">备注模板：人工</span><input v-model="form.remarkManual" type="text" class="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500" /></label>
          <label class="space-y-2"><span class="text-xs font-medium text-gray-600">备注模板：拒绝</span><input v-model="form.remarkReject" type="text" class="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500" /></label>
        </div>
        <div class="space-y-4">
          ...
          <div class="grid gap-4 md:grid-cols-2">
            ...
            <label class="space-y-2"><span class="text-xs font-medium text-gray-600">失败重试次数</span><input v-model.number="form.retryCount" type="number" min="0" class="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500" /></label>
            <label class="space-y-2"><span class="text-xs font-medium text-gray-600">扫描间隔分钟</span><input v-model.number="form.scanIntervalMinutes" type="number" min="1" class="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500" /></label>
          </div>
          <div class="space-y-3"><span class="text-xs font-medium text-gray-600">优先处理类型</span><div class="flex flex-wrap gap-2"><button v-for="type in priorityTypeOptions" :key="type" type="button" class="rounded-full px-3 py-1 text-xs font-medium" :class="form.priorityTypes.includes(type) ? 'bg-brand-900 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'" @click="toggleSelection(form.priorityTypes, type)">{{ type }}</button></div></div>
          ...
          <label class="flex items-center justify-between rounded-md border border-brand-200/50 bg-gray-50 px-4 py-3"><span class="text-sm text-gray-700">启用飞书多维表</span><input v-model="form.bitableEnabled" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-gray-900 focus:ring-brand-500" /></label>
          <label class="space-y-2"><span class="text-xs font-medium text-gray-600">App Token</span><input v-model="form.bitableAppToken" type="text" class="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500" /></label>
          <label class="space-y-2"><span class="text-xs font-medium text-gray-600">Table ID</span><input v-model="form.bitableTableId" type="text" class="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500" /></label>
          <div class="space-y-3"><span class="text-xs font-medium text-gray-600">写入场景</span><div class="flex flex-wrap gap-2"><button v-for="scene in bitableSceneOptions" :key="scene" type="button" class="rounded-full px-3 py-1 text-xs font-medium" :class="form.bitableScenes.includes(scene) ? 'bg-brand-900 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'" @click="toggleSelection(form.bitableScenes, scene)">{{ scene }}</button></div></div>
        </div>
      </section>
    </section>
  </div>
