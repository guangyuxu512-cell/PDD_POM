    ...
    <Modal :show="showEditor" :title="editorTitle" width="min(88vw, 1100px)" @close="closeEditor">
      <form class="space-y-4" @submit.prevent="handleSave">
        <section class="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
          ...
          <div class="space-y-2"><span class="text-xs font-medium text-gray-600">平台</span><Listbox v-model="form.platform"><div class="relative"><ListboxButton class="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-left text-sm text-gray-900">{{ form.platform }}</ListboxButton><ListboxOptions class="absolute z-20 mt-2 w-full rounded-md border border-brand-200/50 bg-white py-1 shadow-lg"><ListboxOption v-for="platform in platformOptions" :key="platform" :value="platform" v-slot="{ active, selected }"><li :class="['cursor-pointer px-3 py-2 text-sm', active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', selected ? 'font-medium text-gray-900' : '']">{{ platform }}</li></ListboxOption></ListboxOptions></div></Listbox></div>
          <div class="space-y-2"><span class="text-xs font-medium text-gray-600">业务类型</span><Listbox v-model="form.business"><div class="relative"><ListboxButton class="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-left text-sm text-gray-900">{{ form.business }}</ListboxButton><ListboxOptions class="absolute z-20 mt-2 w-full rounded-md border border-brand-200/50 bg-white py-1 shadow-lg"><ListboxOption v-for="business in businessOptions" :key="business" :value="business" v-slot="{ active, selected }"><li :class="['cursor-pointer px-3 py-2 text-sm', active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', selected ? 'font-medium text-gray-900' : '']">{{ business }}</li></ListboxOption></ListboxOptions></div></Listbox></div>
        ...
        <section class="border-t border-gray-100 pt-4">
          ...
          <div class="space-y-3">
            <div v-for="condition in form.conditions.rules" :key="condition.id" class="grid gap-3 rounded-md border border-brand-200/50 bg-gray-50 p-3 md:grid-cols-[minmax(0,1fr)_180px_minmax(0,1fr)_84px]">
              ...
              <Listbox v-model="condition.op"><div class="relative"><ListboxButton class="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-left text-sm text-gray-900">{{ condition.op }}</ListboxButton><ListboxOptions class="absolute z-20 mt-2 w-full rounded-md border border-brand-200/50 bg-white py-1 shadow-lg"><ListboxOption v-for="operator in operatorOptions" :key="operator" :value="operator" v-slot="{ active, selected }"><li :class="['cursor-pointer px-3 py-2 text-sm', active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', selected ? 'font-medium text-gray-900' : '']">{{ operator }}</li></ListboxOption></ListboxOptions></div></Listbox>
        ...
        <section class="border-t border-gray-100 pt-4">
          ...
          <div class="space-y-3">
            <div v-for="action in form.actions" :key="action.id" class="grid gap-3 rounded-md border border-brand-200/50 bg-gray-50 p-3 md:grid-cols-[180px_180px_minmax(0,1fr)_84px]">
              <Listbox :modelValue="action.type" @update:modelValue="action.type = $event; handleActionTypeChange(action)"><div class="relative"><ListboxButton class="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-left text-sm text-gray-900">{{ action.type }}</ListboxButton><ListboxOptions class="absolute z-20 mt-2 w-full rounded-md border border-brand-200/50 bg-white py-1 shadow-lg"><ListboxOption v-for="type in actionTypeOptions" :key="type" :value="type" v-slot="{ active, selected }"><li :class="['cursor-pointer px-3 py-2 text-sm', active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', selected ? 'font-medium text-gray-900' : '']">{{ type }}</li></ListboxOption></ListboxOptions></div></Listbox>
              <Listbox v-model="action.action"><div class="relative"><ListboxButton class="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-left text-sm text-gray-900">{{ action.action }}</ListboxButton><ListboxOptions class="absolute z-20 mt-2 w-full rounded-md border border-brand-200/50 bg-white py-1 shadow-lg"><ListboxOption v-for="option in getActionOptions(action.type)" :key="option" :value="option" v-slot="{ active, selected }"><li :class="['cursor-pointer px-3 py-2 text-sm', active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', selected ? 'font-medium text-gray-900' : '']">{{ option }}</li></ListboxOption></ListboxOptions></div></Listbox>
      ...
      <template #footer><button class="rounded-md border border-gray-300 bg-white px-3 py-1.5 text-sm text-gray-700 hover:bg-gray-50" @click="closeEditor">取消</button><button class="rounded-md bg-brand-900 px-3 py-1.5 text-sm text-white hover:bg-brand-700 disabled:opacity-60" :disabled="saving" @click="handleSave">{{ saving ? '保存中...' : '保存' }}</button></template>
    </Modal>
    <Modal :show="showTestMatch" title="测试规则匹配" width="min(72vw, 840px)" @close="closeTestMatchDialog">
      <div class="space-y-4">
        <div class="grid gap-4 md:grid-cols-3">
          <div class="space-y-2"><span class="text-xs font-medium text-gray-600">平台</span><Listbox v-model="testForm.platform"><div class="relative"><ListboxButton class="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-left text-sm text-gray-900">{{ testForm.platform }}</ListboxButton><ListboxOptions class="absolute z-20 mt-2 w-full rounded-md border border-brand-200/50 bg-white py-1 shadow-lg"><ListboxOption v-for="platform in platformOptions" :key="platform" :value="platform" v-slot="{ active, selected }"><li :class="['cursor-pointer px-3 py-2 text-sm', active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', selected ? 'font-medium text-gray-900' : '']">{{ platform }}</li></ListboxOption></ListboxOptions></div></Listbox></div>
          <div class="space-y-2"><span class="text-xs font-medium text-gray-600">业务</span><Listbox v-model="testForm.business"><div class="relative"><ListboxButton class="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-left text-sm text-gray-900">{{ testForm.business }}</ListboxButton><ListboxOptions class="absolute z-20 mt-2 w-full rounded-md border border-brand-200/50 bg-white py-1 shadow-lg"><ListboxOption v-for="business in businessOptions" :key="business" :value="business" v-slot="{ active, selected }"><li :class="['cursor-pointer px-3 py-2 text-sm', active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', selected ? 'font-medium text-gray-900' : '']">{{ business }}</li></ListboxOption></ListboxOptions></div></Listbox></div>
        ...
        <div class="rounded-md border border-brand-200/50 bg-gray-50 p-4">
          ...
          <div v-if="matchResult" class="mt-4 space-y-2 rounded-md border border-brand-200/50 bg-white p-4 text-sm text-gray-700">
