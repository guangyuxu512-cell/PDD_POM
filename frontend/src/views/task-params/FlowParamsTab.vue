...
<template>
  <div class="flex flex-col gap-3">
    ...
    <div class="flex flex-wrap items-center gap-2 rounded-md border border-brand-300/50 bg-white px-4 py-3 shadow-sm">
      ...
      <span class="ml-auto text-xs text-brand-500">流程参数支持按流程、状态、店铺筛选后批量重置、启用和禁用。</span>
    </div>

    <!-- 表格 -->
    <div class="overflow-x-auto rounded-md border border-brand-300/50 bg-white shadow-sm">
      <table class="min-w-[1328px] w-full table-fixed divide-y divide-brand-300/30">
        ...
        <tbody class="divide-y divide-brand-300/20 text-sm text-gray-900">
          <tr v-if="loading">
            <td colspan="11" class="px-4 py-8 text-center text-sm text-brand-500">加载中...</td>
          </tr>
          <tr v-else-if="flowParams.length === 0">
            <td colspan="11" class="px-4 py-8 text-center text-sm text-brand-500">暂无流程参数记录</td>
          </tr>
          <template v-else>
            <tr
              v-for="flowParam in flowParams"
              :key="flowParam.id"
              :class="['transition hover:bg-brand-100/50', flowParam.enabled ? '' : 'opacity-50']"
            >
              <td class="px-4 py-3 text-center font-mono text-xs text-brand-500">{{ flowParam.id }}</td>
              ...
              <td class="px-4 py-3 text-left">
                <div class="flex flex-wrap gap-1" :title="store.formatStepResultsSummary(flowParam.step_results)">
                  <button
                    ...
                    type="button"
                    :class="['step-result-tag rounded-full px-2.5 py-1 text-xs font-bold transition hover:opacity-80', store.getStepResultStatusClass(step.status)]"
                    @click="store.toggleStepResultDetail(flowParam.id, step.name)"
                  >
                    {{ store.formatStepResultTag(step) }}
                  </button>
                  <span v-if="store.getStepResultItems(flowParam.step_results).length === 0" class="text-brand-500">-</span>
                  <div
                    v-for="step in store.getStepResultItems(flowParam.step_results)"
                    v-show="store.isStepResultDetailOpen(flowParam.id, step.name)"
                    :key="`${flowParam.id}-${step.name}-detail`"
                    class="mt-1 w-full rounded-md border border-brand-300/50 bg-brand-100 p-2"
                  >
                    <strong class="mb-1 block text-xs text-brand-700">{{ step.name }}</strong>
                    <pre class="whitespace-pre-wrap break-words text-xs text-brand-500">{{ store.formatJsonTooltip(step.detail) }}</pre>
                  </div>
                </div>
              </td>
              ...
              <td class="px-4 py-3 text-right font-mono text-xs text-brand-500">{{ store.formatDateTime(flowParam.created_at) }}</td>
