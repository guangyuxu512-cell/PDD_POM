...
const statusConfig = computed<StatusConfig>(() => {
  if (props.type === 'shop') {
    const configs: Record<string, StatusConfig> = {
      ...
      offline: { text: '离线', dotClass: 'bg-brand-300', textClass: 'text-brand-500' },
  ...
  if (props.type === 'task') {
    const configs: Record<string, StatusConfig> = {
      pending: { text: '等待中', dotClass: 'bg-brand-300', textClass: 'text-brand-500' },
      waiting: { text: '等待中', dotClass: 'bg-brand-300', textClass: 'text-brand-500' },
      ...
      stopped: { text: '已停止', dotClass: 'bg-brand-300', textClass: 'text-brand-700' },
      skipped: { text: '跳过', dotClass: 'bg-brand-300', textClass: 'text-brand-700' },
      cancelled: { text: '已取消', dotClass: 'bg-brand-300', textClass: 'text-brand-700' },
    }
  ...
  const configs: Record<string, StatusConfig> = {
    INFO: { text: 'INFO', dotClass: 'bg-brand-300', textClass: 'text-brand-700' },
