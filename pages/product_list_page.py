...
class 商品列表页(基础页):
    ...
    async def 导航(self) -> None:
        ...
        logger.info(f"[商品列表页] 当前URL: {self.页面.url}")
        await self.随机延迟(1, 2)
        await self.关闭所有弹窗()
        logger.info(f"[商品列表页] 当前URL: {self.页面.url}")
    ...
    async def 导航到商品列表(self) -> None:
        """导航到商品列表页 URL。"""
        await self.页面.goto(self.页面地址, wait_until="domcontentloaded")
        logger.info(f"[商品列表页] 商品列表页加载完成: {self.页面.url}")
        await self.页面加载延迟()
    ...
    async def _保存搜索失败截图(self) -> None:
        ...
        try:
            ...
            logger.info(f"[商品列表页] 已保存失败截图: {截图路径}")
        except Exception as 异常:
            logger.info(f"[商品列表页] 保存失败截图失败: {异常}")

    async def 等待搜索结果(self) -> None:
        """等待列表刷新完成。"""
        logger.info("[商品列表页] 准备等待搜索结果刷新")
        等待方法 = getattr(self.页面, "wait_for_selector", None)
        if 等待方法 is None:
            await self.随机延迟(1.0, 2.0)
            logger.info("[商品列表页] 页面未提供 wait_for_selector，已使用随机延迟代替")
            return
        ...
        for 选择器 in 候选选择器:
            try:
                ...
                if inspect.isawaitable(等待结果):
                    await 等待结果
                await self.随机延迟(1.0, 2.0)
                logger.info(f"[商品列表页] 搜索结果刷新完成: {选择器}")
                return
            except Exception as 异常:
                最后异常 = 异常
                logger.info(f"[商品列表页] 搜索结果等待失败({选择器}): {异常}")

        await self.随机延迟(1.0, 2.0)
        logger.info(f"[商品列表页] 搜索结果未命中候选选择器，已回退随机延迟: {最后异常}")
    ...
    async def 输入商品ID(self, 商品ID: str) -> None:
        ...
        for 选择器 in 商品列表页选择器.商品ID搜索框.所有选择器():
            try:
                logger.info(f"[商品列表页] 尝试定位商品ID输入框: {选择器}")
                输入框 = self.页面.locator(选择器).first
                await 输入框.click(timeout=10000)
                await self.随机延迟(0.2, 0.5)
                await 输入框.fill(商品ID)
                await self.操作后延迟()
                logger.info(f"[商品列表页] 商品ID填写完成: {选择器}")
                return
            except Exception as 异常:
                最后异常 = 异常
                logger.info(f"[商品列表页] 商品ID输入框定位失败({选择器}): {异常}")
    ...
    async def 点击查询(self) -> None:
        ...
        for 选择器 in 商品列表页选择器.查询按钮.所有选择器():
            try:
                ...
                await self.操作后延迟()
                logger.info(f"[商品列表页] 查询按钮点击完成: {选择器}")
                return
            except Exception as 异常:
                logger.info(f"[商品列表页] 查询按钮点击失败({选择器}): {异常}")
    ...
    async def 点击发布相似(self) -> None:
        ...
        for 选择器 in 商品列表页选择器.发布相似按钮.所有选择器():
            try:
                await self.页面.click(选择器, timeout=10000)
                await self.操作后延迟()
                logger.info(f"[商品列表页] 发布相似品按钮点击完成: {选择器}")
                return
            except Exception as 异常:
                logger.info(f"[商品列表页] 发布相似品按钮点击失败({选择器}): {异常}")
    ...
    async def 确认发布相似弹窗(self) -> None:
        ...
        for 选择器 in 商品列表页选择器.发布相似品弹窗_确认按钮.所有选择器():
            try:
                await self.页面.click(选择器, timeout=10000)
                await self.页面加载延迟()
                logger.info(f"[商品列表页] 发布相似品弹窗确认完成: {选择器}")
                return
            except Exception as 异常:
                logger.info(f"[商品列表页] 发布相似品弹窗确认失败({选择器}): {异常}")
    ...
    async def 点击发布相似品(self):
        ...
        logger.info(f"[商品列表页] 已拿到新页面, URL: {新页面.url}")
        logger.info(f"[商品列表页] 准备等待编辑页URL, URL: {新页面.url}")
        ...
        if inspect.isawaitable(等待结果):
            await 等待结果
        logger.info(f"[商品列表页] 编辑页URL等待完成, URL: {新页面.url}")
        logger.info(f"[商品列表页] 准备等待新页面加载完成, URL: {新页面.url}")
        await 新页面.wait_for_load_state("domcontentloaded")
        logger.info(f"[商品列表页] 新页面加载完成, URL: {新页面.url}")
        return 新页面
