...
- *.pyc                 # Python字节码文件
- data/profiles/        # 浏览器缓存目录
- node_modules/         # 前端依赖（AI无需分析）
- .env                  # 配置文件（无代码逻辑）
- *.log                 # 日志文件
- *.db                  # 数据库文件（二进制/大文件）
- tmp/                  # 临时文件目录
...
1. **Page 层每个 def 只做一个页面操作**（点一个按钮、填一个输入框、等一个状态）
2. **不要把多个操作合成一个大方法**，流程编排是 Task 层的职责
3. **不要自行添加流程步骤**，严格按照 Task 描述的步骤实现
4. **不要在 Page 层写业务逻辑**（判断是否跳过、读取参数等都在 Task 层）
5. **每个方法内部用 `for sel in 选择器.所有选择器()` 遍历 fallback**
6. **不要在选择器层添加任何 Task 描述中没提到的选择器配置**