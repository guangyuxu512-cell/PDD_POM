...
class 限时限量任务(基础任务):
    ...
    def _构建执行结果(self, 批次ID: str, 折扣值: float, 新商品ID: str) -> dict[str, Any]:
        """统一构建任务结果。"""
        结果 = {
            "折扣": 折扣值,
            "新商品ID": 新商品ID,
    ...
    async def 执行(self, 页面, 店铺配置: dict) -> str:
        ...
        折扣值 = self._读取折扣(任务参数)
        self._执行结果 = self._构建执行结果(批次ID, 折扣值, 新商品ID)
        ...
        try:
            ...
            await 上报(f"选择商品: {新商品ID}", 店铺ID)
            await 页面对象.弹窗输入商品ID(新商品ID)
            await 页面对象.弹窗点击查询()
            await 页面对象.弹窗等待结果()
            await 页面对象.弹窗勾选第一行()
            ...
            if await 页面对象.等待创建成功():
                ...
                await 上报(f"[成功] 限时限量创建成功，商品ID: {新商品ID}", 店铺ID)
                return "成功"
