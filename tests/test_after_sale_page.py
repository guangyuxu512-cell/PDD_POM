...
class 测试_售后页:
    ...
    async def test_导航到售后列表_打开校准URL(self, 模拟页面):
        from pages.after_sale_page import 售后页
    ...
    async def test_批量抓取当前页_返回当前页所有售后单(self, 模拟页面):
        ...
        with patch("pages.after_sale_page.asyncio.sleep", new_callable=AsyncMock):
    ...
    async def test_详情页截图_保存到截图目录(self, 模拟页面, tmp_path):
        ...
        with patch("pages.after_sale_page.配置实例") as 配置实例模拟:
