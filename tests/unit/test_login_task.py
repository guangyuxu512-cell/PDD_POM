...
class 测试_登录任务:
    ...
    async def test_手机号密码登录成功(self, 模拟回调, 模拟页面, 模拟登录页):
        ...
        from tasks.login_task import 登录任务

        with patch("tasks.login_task.登录页", return_value=模拟登录页):
            任务 = 登录任务()
            结果 = await 任务.执行(
                模拟页面,
    ...
    async def test_短信验证码验证通过后登录成功(self, 模拟回调, 模拟页面, 模拟登录页):
        ...
        with patch("tasks.login_task.登录页", return_value=模拟登录页), patch(
            "tasks.login_task.asyncio.sleep", new_callable=AsyncMock
        ) as 模拟睡眠:
