...
from backend.api.task_api import 路由
...
class 测试_任务接口内部执行:
    ...
    def test_execute_internal_成功返回统一响应并回写流程结果(self, 客户端: TestClient):
        with patch(
            "backend.api.task_api.任务服务.创建任务记录",
            new=AsyncMock(return_value={"task_id": "task-1"}),
        ), patch(
            "backend.api.task_api.任务服务.统一执行任务",
            ...
            "backend.api.task_api.流程参数服务实例.更新",
            new=AsyncMock(return_value={}),
        ) as 模拟更新, patch(
            "backend.api.task_api.流程参数服务实例.更新步骤结果",
            new=AsyncMock(return_value={}),
        ) as 模拟更新步骤结果, patch(
            "backend.api.task_api.流程参数服务实例.获取步骤上下文",
            new=AsyncMock(return_value={"discount": 6}),
        ) as 模拟获取上下文, patch(
            "backend.api.task_api.任务服务.处理流程步骤执行完成",
            new=AsyncMock(return_value={}),
        ) as 模拟处理流程:
            响应 = 客户端.post(
    ...
    def test_execute_internal_flow_mode会读取运行上下文并在abort失败时不续投递(self, 客户端: TestClient):
        with patch(
            ...
            "backend.api.task_api.运行服务实例.获取运行项流程上下文",
            new=AsyncMock(return_value={"parent_product_id": "9001"}),
        ), patch(
            "backend.api.task_api.运行服务实例.回写无流程参数步骤结果",
            new=AsyncMock(return_value={
                "next_step": {"task": "推广", "on_fail": "abort", "merge": False},
                "total_steps": 2,
            }),
        ) as 模拟回写步骤结果, patch(
            "backend.api.task_api.执行服务实例.投递单步任务",
            new=AsyncMock(),
        ) as 模拟投递下一步:
            响应 = 客户端.post(
                "/api/tasks/execute-internal",
    ...
    def test_execute_internal_flow_mode成功后由主进程投递下一步(self, 客户端: TestClient):
        with patch(
            ...
            "backend.api.task_api.执行服务实例.获取最新批次状态",
            new=AsyncMock(return_value={
                "batch_id": "batch-3",
