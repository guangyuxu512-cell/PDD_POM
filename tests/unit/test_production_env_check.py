...
from backend.services import scheduled_execute_service as 定时执行服务模块
from backend.services.shop_service import 店铺服务, 店铺服务实例
...
class 测试_SSE保活:
    ...
    def test_执行状态流_保活事件输出_ping(self):
        ...
        with TestClient(应用) as 客户端, patch(
            "backend.api.execute_api.执行服务实例.订阅批次状态",
            new=假订阅批次状态,
        ):
    ...
    def test_日志流推送_空闲时输出_ping(self):
        ...
        with TestClient(应用) as 客户端, patch(
            "backend.api.log_api.日志流心跳间隔秒",
            0.01,
        ), patch(
            "backend.api.log_api.日志服务实例.订阅",
            return_value=假队列(),
        ), patch(
            "backend.api.log_api.日志服务实例.取消订阅",
            new=MagicMock(),
        ) as 模拟取消订阅:
...
def test_Electron生产打包_改为嵌入式Python源码方案():
    """生产模式应使用 PyInstaller 打包的 exe，而不是嵌入式 Python 源码方案。"""
    主进程文件 = Path("electron/main.js").read_text(encoding="utf-8")
    打包配置 = Path("electron/package.json").read_text(encoding="utf-8")

    assert "packagedBackendExe" in 主进程文件
    assert "packagedCeleryExe" in 主进程文件
    assert "python-embed" not in 主进程文件
    assert "../python-backend-dist" in 打包配置
    assert "../dist_bundle" not in 打包配置
