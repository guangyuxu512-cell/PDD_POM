...
import browser.task_callback as 任务回调模块
...
class 测试_任务回调:
    ...
    async def test__回调_携带_X_RPA_KEY_并校验业务响应(self):
        ...
        try:
            with patch.object(任务回调模块.配置实例, "X_RPA_KEY", "test-rpa-key"), \
                    patch("browser.task_callback.httpx.AsyncClient", return_value=客户端实例):
    ...
    async def test__回调_业务失败时静默忽略(self):
        ...
        try:
            with patch.object(任务回调模块.配置实例, "X_RPA_KEY", "test-rpa-key"), \
                    patch("browser.task_callback.httpx.AsyncClient", return_value=假客户端()):
