...
class 测试_任务服务浏览器复用与后续任务:
    ...
    async def test_统一执行任务_主页面已关闭时自动创建新页面(self):
        ...
        with patch(
            "backend.services.task_service.任务服务实例.更新任务状态",
            new=AsyncMock(),
        ), patch(
            "backend.services.browser_service.确保已初始化",
            new=AsyncMock(),
        ), patch(
            "backend.services.browser_service.获取当前管理器实例",
            return_value=管理器,
        ), patch(
            "backend.services.shop_service.店铺服务实例.根据ID获取完整信息",
    ...
    async def test_执行任务_发布相似商品成功后创建下一步任务(self):
        ...
        with patch(
            ...
            "backend.services.task_service.任务参数服务实例.创建后续任务",
