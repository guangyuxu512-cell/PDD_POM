...
class 测试_执行任务:
    ...
    def test_continue策略_失败后继续后续步骤(self):
        ...
        with patch("tasks.execute_task.初始化Worker环境"), \
                ...
                patch("tasks.execute_task.同步更新批次店铺状态") as 模拟更新批次状态, \
                patch("tasks.execute_task.同步读取批次状态", return_value=批次状态), \
                patch("tasks.execute_task.同步检查取消标记", return_value=False), \
    ...
    def test_HTTP返回后检测到取消标记则返回cancelled(self):
        ...
        with patch("tasks.execute_task.初始化Worker环境"), \
                patch("tasks.execute_task.获取任务类"), \
                patch("tasks.execute_task.httpx.Client", return_value=客户端上下文), \
                patch("tasks.execute_task.同步检查取消标记", return_value=True), \
                patch("tasks.execute_task.同步更新批次店铺状态") as 模拟更新批次状态:
            结果 = 执行任务函数(
                假任务对象,
                batch_id="batch-1",
    ...
    def test_批次已停止时不投递下一步(self):
        ...
        with patch("tasks.execute_task.初始化Worker环境"), \
                patch("tasks.execute_task.获取任务类"), \
                patch("tasks.execute_task.httpx.Client", return_value=客户端上下文), \
                patch("tasks.execute_task.同步读取批次状态", return_value={"stopped": True, "shops": {"shop-1": {"steps": [{"task": "登录"}, {"task": "采集商品"}]}}}), \
                patch("tasks.execute_task.同步检查取消标记", return_value=False), \
                patch("tasks.execute_task.同步更新批次店铺状态"), \
