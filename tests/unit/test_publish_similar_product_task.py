...
class 测试_发布相似商品任务:
    ...
    def test_注册名为_发布相似商品(self):
        ...
        import tasks.publish_similar_product_task as 发布相似商品任务模块
    ...
    @pytest.mark.asyncio
    @patch("browser.task_callback._回调", new_callable=AsyncMock)
    async def test_缺少_父商品ID_抛ValueError(self, 模拟回调, 模拟页面):
        ...
        with patch("tasks.publish_similar_product_task.上报", new_callable=AsyncMock), \
                patch("tasks.publish_similar_product_task.任务参数服务实例.更新执行结果", new_callable=AsyncMock):
    ...
    async def test_正常流程_英文参数成功并写入中文结果(
        ...
        with patch("tasks.publish_similar_product_task.上报", new_callable=AsyncMock), \
                patch("tasks.publish_similar_product_task.商品列表页", return_value=模拟商品列表页), \
                patch("tasks.publish_similar_product_task.发布商品页", return_value=模拟发布页), \
                patch("tasks.publish_similar_product_task.random.randint", return_value=2), \
                patch("tasks.publish_similar_product_task.任务参数服务实例.更新执行结果", new_callable=AsyncMock) as 模拟回填:
            任务 = 发布相似商品任务()
            任务._步骤间延迟 = AsyncMock()
            结果 = await 任务.执行(
                模拟页面,
