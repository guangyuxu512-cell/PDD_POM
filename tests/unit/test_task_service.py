...
class 测试_任务服务:
    ...
    async def test_执行任务_发布相似商品自动注入首条待执行参数并回填成功(self):
        ...
        with patch(
            "backend.services.task_service.任务参数服务实例.获取待执行列表",
            new=AsyncMock(return_value=[
                {"id": 11, "params": {"parent_product_id": "916453776556", "new_title": "测试标题"}},
                {"id": 12, "params": {"parent_product_id": "916453776557"}},
            ]),
        ), patch(
            "backend.services.task_service.任务参数服务实例.更新执行结果",
            new=AsyncMock(),
        ) as 模拟回填, patch(
            "tasks.task_registry.获取任务",
            return_value=假任务实例,
        ):
    ...
    async def test_执行任务_收到取消信号时直接返回_cancelled(self):
        ...
        with patch(
            "backend.services.execute_service.检查取消标记",
    ...
    async def test_执行任务_flow_param_ids_barrier模式复用同一页面循环执行(self):
        ...
        with patch(
            "backend.services.flow_params_service.流程参数服务实例.更新",
            new=AsyncMock(return_value={}),
        ), patch(
            "backend.services.flow_params_service.流程参数服务实例.更新步骤结果",
            new=AsyncMock(return_value={}),
        ), patch(
            "backend.services.flow_params_service.流程参数服务实例.获取步骤上下文",
            new=AsyncMock(side_effect=[
                {"parent_product_id": "9001", "discount": 6},
                {"parent_product_id": "9002", "discount": 7},
            ]),
        ) as 模拟获取上下文, patch(
            "backend.services.task_service.任务服务实例.处理流程步骤执行完成",
            new=AsyncMock(),
        ) as 模拟处理流程, patch(
            "backend.services.task_service.asyncio.sleep",
            new=AsyncMock(),
        ) as 模拟睡眠, patch(
            "tasks.task_registry.获取任务",
            side_effect=[任务一, 任务二],
    ...
    async def test_执行任务_flow_param_ids_merge模式只执行一次(self):
        ...
        with patch(
            "backend.services.flow_params_service.流程参数服务实例.根据ID获取",
            new=AsyncMock(side_effect=[
            ...
            "backend.services.task_service.任务服务实例._构建合并参数",
            new=AsyncMock(return_value={
                "商品ID列表": ["1001", "1002"],
                "商品参数映射": {
            ...
            "backend.services.task_service.任务服务实例._标记合并跳过",
            new=AsyncMock(),
        ) as 模拟标记跳过, patch(
            "backend.services.task_service.任务服务实例.处理流程步骤执行完成",
            new=AsyncMock(),
    ...
    async def test_执行任务_flow_param_ids_abort时后续记录标记失败(self):
        ...
        with patch(
            ...
            "backend.services.flow_params_service.流程参数服务实例.更新执行状态",
            new=AsyncMock(return_value={}),
        ) as 模拟更新执行状态, patch(
            "backend.services.flow_params_service.流程参数服务实例.获取步骤上下文",
    ...
    async def test_统一执行任务_浏览器初始化超时返回失败(self):
        ...
        with patch("backend.services.task_service.任务服务实例.更新任务状态", new=AsyncMock()) as 模拟更新任务状态, \
                patch("backend.services.shop_service.店铺服务实例.更新", new=AsyncMock()) as 模拟更新店铺状态, \
                patch("backend.services.browser_service.确保已初始化", new=AsyncMock(side_effect=asyncio.TimeoutError())), \
                patch("backend.services.browser_service.管理器实例", None):
    ...
    async def test_统一执行任务_执行前检测到取消则写为_cancelled(self):
        with patch("backend.services.task_service.任务服务实例.更新任务状态", new=AsyncMock()) as 模拟更新任务状态, \
                patch("backend.services.shop_service.店铺服务实例.根据ID获取完整信息", new=AsyncMock(return_value={
                    "id": "shop-1",
                    "name": "测试店铺",
                    "username": "demo",
                    "password": "pwd",
                })), \
                patch("backend.services.browser_service.确保已初始化", new=AsyncMock()), \
                patch("backend.services.browser_service.获取当前管理器实例", return_value=假管理器()), \
                patch("backend.services.execute_service.检查取消标记", new=AsyncMock(return_value=True)), \
                patch("backend.services.task_service.任务服务实例._确保页面可用", new=AsyncMock(return_value=object())), \
                patch("backend.services.task_service.任务服务实例.执行任务", new=AsyncMock(return_value={"result": "成功", "result_data": {}})) as 模拟执行任务:
            结果 = await 任务服务实例.统一执行任务(
                task_id="task-1",
                shop_id="shop-1",
                task_name="发布相似商品",
    ...
    async def test_统一执行任务_请求参数中的_flow_context会注入到店铺配置(self):
        with patch("backend.services.task_service.任务服务实例.更新任务状态", new=AsyncMock()), \
                patch("backend.services.browser_service.确保已初始化", new=AsyncMock()), \
                patch("backend.services.browser_service.获取当前管理器实例", return_value=假管理器()), \
                patch("backend.services.task_service.任务服务实例._确保页面可用", new=AsyncMock(return_value=object())), \
                patch("backend.services.execute_service.检查取消标记", new=AsyncMock(return_value=False)), \
                patch("backend.services.shop_service.店铺服务实例.根据ID获取完整信息", new=AsyncMock(return_value={
                    "id": "shop-1",
                    "name": "测试店铺",
                    "username": "demo",
                ...
                patch("backend.services.shop_service.店铺服务实例.更新", new=AsyncMock()), \
                patch(
                    "backend.services.task_service.任务服务实例.执行任务",
