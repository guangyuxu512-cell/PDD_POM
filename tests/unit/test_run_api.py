...
from backend.services.execute_service import 执行服务, 同步写入运行实例状态
...
class 测试_运行接口:
    ...
    def test_取消运行接口_透传到执行服务(self, 客户端: TestClient):
        with patch(
            "backend.api.run_api.执行服务实例.停止批次",
    ...
    def test_运行事件接口输出SSE流(self, 客户端: TestClient):
        ...
        with patch("backend.api.run_api.执行服务实例.订阅批次状态", new=假订阅批次状态):
    ...
    def test_重试失败项接口会转发到运行服务(self, 客户端: TestClient):
        with patch(
            "backend.api.run_api.运行服务实例.重试失败项",
