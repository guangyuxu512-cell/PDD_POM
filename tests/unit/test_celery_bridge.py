...
from tasks.bridge_task import 桥接执行任务, _运行异步任务
...
class 测试_Celery桥接:
    ...
    def test_初始化Worker环境_创建复用事件循环并设置回调(self):
        ...
        with patch("tasks.task_registry.初始化任务注册表") as 模拟初始化任务注册表, \
                ...
                patch("tasks.celery_app.注册Worker机器") as 模拟注册Worker机器, \
    ...
    def test_初始化Worker环境_向Agent注册机器(self):
        ...
        with patch("tasks.task_registry.初始化任务注册表"), \
                patch("browser.task_callback.设置回调地址"), \
                patch.object(celery应用模块.配置实例, "AGENT_CALLBACK_URL", "http://agent/callback"), \
                ...
                patch("tasks.celery_app.httpx.Client") as 模拟客户端类:
    ...
    def test_Worker信号钩子_初始化异常时只记录不抛出(self):
        ...
        with patch("tasks.celery_app.初始化Worker环境", side_effect=RuntimeError("init failed")):
            celery应用模块.Worker启动时初始化()

        with patch("tasks.celery_app.关闭Worker环境", side_effect=RuntimeError("shutdown failed")):
