...
class 测试_发布商品页:
    ...
    async def test_获取主图列表与拖拽主图(self, 模拟页面):
        ...
        with patch("pages.publish_product_page.random.uniform", return_value=0), \
                patch("pages.publish_product_page.random.randint", return_value=8):
            await 页面对象.拖拽主图(2, 0)
    ...
    async def test_随机调整主图到第一位_多图拖拽(self, 模拟页面):
        ...
        with patch("pages.publish_product_page.random.randint", return_value=2):
