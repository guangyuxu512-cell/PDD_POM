...
from backend.services.heartbeat_service import 心跳服务
...
class 测试_心跳服务:
    ...
    async def test_发送心跳_按Agent约定发送请求头和shadowbot_running(self):
        ...
        with patch("backend.services.heartbeat_service.配置实例.AGENT_MACHINE_ID", "machine-a"), \
                patch("backend.services.heartbeat_service.配置实例.AGENT_HEARTBEAT_URL", "http://agent/heartbeat"), \
                patch("backend.services.heartbeat_service.配置实例.X_RPA_KEY", "test-rpa-key"), \
                patch("backend.services.heartbeat_service.httpx.AsyncClient", return_value=客户端实例):
            await 服务._发送心跳()
    ...
    async def test_发送心跳_HTTP异常时静默忽略(self):
        ...
        with patch("backend.services.heartbeat_service.配置实例.AGENT_MACHINE_ID", "machine-a"), \
                patch("backend.services.heartbeat_service.配置实例.AGENT_HEARTBEAT_URL", "http://agent/heartbeat"), \
                patch("backend.services.heartbeat_service.配置实例.X_RPA_KEY", "test-rpa-key"), \
                patch("backend.services.heartbeat_service.httpx.AsyncClient", return_value=假客户端()):
            await 服务._发送心跳()
