...
class 测试_推广任务:
    ...
    def test_注册名为_设置推广(self):
        ...
        import tasks.promotion_task as 推广任务模块
    ...
    async def test_缺少商品ID列表_抛出_ValueError(self, 模拟回调, 模拟页面):
        from tasks.promotion_task import 推广任务

        任务 = 推广任务()

        with patch("tasks.promotion_task.上报", new_callable=AsyncMock):
    ...
    async def test_正常流程_返回成功并记录结果(self, 模拟回调, 模拟页面, 模拟推广页):
        ...
        with patch("tasks.promotion_task.上报", new_callable=AsyncMock), \
                patch("tasks.promotion_task.推广页", return_value=模拟推广页):
            任务 = 推广任务()
            结果 = await 任务.执行(
                模拟页面,
                {
