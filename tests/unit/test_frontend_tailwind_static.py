...
class 测试_Tailwind接入:
    def test_vite与样式入口切换到_tailwind_brand_主题(self):
        ...
        assert "@theme {" in 全局样式
        assert "--color-brand-950: #06141B;" in 全局样式
        assert "--color-brand-100: #CCD0CF;" in 全局样式
    ...
    def test_中等页面与组件统一为_brand_Tailwind结构(self):
        断言映射 = {
            ...
            "frontend/src/views/TaskMonitor.vue": [
                "手动触发",
                "StatusBadge",
                "bg-brand-100 text-xs font-medium uppercase tracking-wider text-brand-500",
            ],
            ...
            "frontend/src/components/StatCard.vue": [
                ...
                "text-xs font-medium uppercase tracking-wider text-brand-500",
            ],
            "frontend/src/components/LogTable.vue": [
                "overflow-hidden rounded-md border border-brand-300/50 bg-white shadow-sm",
                "font-mono text-xs uppercase tracking-wide text-brand-500",
            ],
