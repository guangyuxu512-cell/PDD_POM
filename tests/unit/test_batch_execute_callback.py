...
from backend.api.execute_api import 路由
...
class 测试_批量执行回调:
    ...
    def test_同步写入批次状态_回调失败不影响主流程(self):
        ...
        with patch("backend.services.execute_service.同步获取Redis客户端", return_value=假客户端), \
                patch("backend.services.execute_service.发送批次完成回调", side_effect=RuntimeError("callback failed")):
    ...
    def test_发送批次完成回调_默认地址取Agent根地址并携带鉴权头(self):
        ...
        with patch.object(执行服务模块.配置实例, "AGENT_CALLBACK_URL", "http://agent-host:8001/api/task-callback"), \
                ...
                patch("backend.services.execute_service.httpx.Client") as 模拟客户端类:
    ...
    def test_批量执行接口_透传_callback_url(self, 客户端: TestClient):
        ...
        with patch(
            "backend.api.execute_api.执行服务实例.创建批次",
