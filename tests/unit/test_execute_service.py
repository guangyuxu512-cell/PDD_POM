...
from backend.services import execute_service as 执行服务模块
...
class 测试_执行服务:
    ...
    async def test_创建批次_流程模式先写入状态再投递首步任务(self):
        ...
        with patch.object(执行服务模块.配置实例, "AGENT_MACHINE_ID", "machine-1"), \
                ...
                patch("backend.services.execute_service.店铺服务实例.根据ID获取", new=AsyncMock(side_effect=lambda 店铺ID: {"id": 店铺ID})), \
                patch(
    ...
    async def test_创建批次_流程模式无待执行流程参数时仍创建空上下文任务(self):
        ...
        with patch.object(执行服务模块.配置实例, "AGENT_MACHINE_ID", "machine-1"), \
                ...
                patch("backend.services.execute_service.流程参数服务实例.更新", new=AsyncMock()), \
                patch.object(服务, "_写入运行实例快照", new=AsyncMock()), \
                patch.object(服务, "投递单步任务", new=AsyncMock(side_effect=假投递单步任务)), \
                patch.object(服务, "_写入批次状态", new=AsyncMock(side_effect=假写入批次状态)):
            结果 = await 服务.创建批次(
    ...
    async def test_创建批次_输入集模式会透传_input_set_id_并创建兼容流程参数(self):
        ...
        with patch.object(执行服务模块.配置实例, "AGENT_MACHINE_ID", "machine-1"), \
                patch("backend.services.execute_service.店铺服务实例.根据ID获取", new=AsyncMock(return_value={"id": "shop-1", "name": "店铺一"})), \
                patch.object(
                    服务,
                    "_构建步骤列表",
    ...
    async def test_创建批次_require_input缺少输入时直接报错(self):
        ...
        with patch("backend.services.execute_service.店铺服务实例.根据ID获取", new=AsyncMock(return_value={"id": "shop-1"})), \
                patch.object(
                    服务,
                    "_构建步骤列表",
                    new=AsyncMock(return_value=[{"task": "发布相似商品", "on_fail": "abort", "barrier": False, "merge": False}]),
                ...
                patch(
                    "backend.services.execute_service.获取任务元数据",
    ...
    @pytest.mark.asyncio
    async def test_预检流程_会执行input_schema类型校验(self):
        """预检应基于 input_schema 拦截类型不匹配的参数。"""
        服务 = 执行服务模块.执行服务()
        ...
        with patch("backend.services.execute_service.店铺服务实例.根据ID获取", new=AsyncMock(return_value={"id": "shop-1"})), \
                patch.object(
                    服务,
                    "_构建步骤列表",
                    new=AsyncMock(return_value=[{"task": "接口任务", "on_fail": "abort", "barrier": False, "merge": False}]),
                ), \
                patch.object(
                    服务,
                    "_获取流程输入行映射",
                    new=AsyncMock(return_value={"shop-1": [{"id": 11, "shop_id": "shop-1", "input_data": {"retry_count": "bad"}}]}),
                ), \
                patch(
                    "backend.services.execute_service.获取任务元数据",
                    return_value={
                        "requires_input": False,
                        "required_fields": ["retry_count"],
        ...
        assert 结果["summary"]["failed_items"] == 1
        assert 结果["items"][0]["input_row_id"] == 11
        assert "参数校验失败" in str(结果["items"][0]["error"])
    ...
    async def test_创建批次_未注册任务返回错误(self):
        ...
        with patch("backend.services.execute_service.初始化任务注册表"), \
                patch("backend.services.execute_service.获取任务类", side_effect=KeyError("missing")), \
                patch("backend.services.execute_service.店铺服务实例.根据ID获取", new=AsyncMock(return_value={"id": "shop-1"})):
    ...
    async def test_停止批次_撤销任务并更新批次状态(self):
        ...
        with patch.object(服务, "获取最新批次状态", new=AsyncMock(return_value=原始批次.copy())), \
                patch.object(服务, "_写入批次状态", new=AsyncMock(side_effect=假写入批次状态)), \
                patch("backend.services.execute_service.设置取消标记", new=AsyncMock(return_value=True)) as 模拟设置取消标记, \
                patch("backend.services.execute_service.celery_app.control.revoke") as 模拟撤销:
            结果 = await 服务.停止批次()
