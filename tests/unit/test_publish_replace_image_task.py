...
class 测试_发布换图商品任务:
    ...
    def test_注册名为_发布换图商品(self):
        ...
        import tasks.publish_replace_image_task as 发布换图商品任务模块
    ...
    async def test_image_path和new_title为空时跳过上传与改标题(
        ...
        from tasks.publish_replace_image_task import 发布换图商品任务

        模拟页面.expect_popup.return_value = 模拟弹窗上下文(模拟发布页面)
        with patch("tasks.publish_replace_image_task.上报", new_callable=AsyncMock), \
                patch("tasks.publish_replace_image_task.商品列表页", return_value=模拟商品列表页), \
                patch("tasks.publish_replace_image_task.发布商品页", return_value=模拟发布页), \
                patch("tasks.publish_replace_image_task.任务参数服务实例.更新执行结果", new_callable=AsyncMock):
            任务 = 发布换图商品任务()
            结果 = await 任务.执行(
                模拟页面,
                {
    ...
    async def test_正常流程_返回成功(
        ...
        with patch("tasks.publish_replace_image_task.上报", new_callable=AsyncMock), \
                patch("tasks.publish_replace_image_task.商品列表页", return_value=模拟商品列表页), \
                patch("tasks.publish_replace_image_task.发布商品页", return_value=模拟发布页), \
                patch("tasks.publish_replace_image_task.任务参数服务实例.更新执行结果", new_callable=AsyncMock) as 模拟回填:
            任务 = 发布换图商品任务()
            结果 = await 任务.执行(
                模拟页面,
                {
