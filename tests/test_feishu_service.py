...
from backend.services.feishu_service import 飞书服务
...
class 测试_飞书服务:
    ...
    async def test_发送文本通知_正确组装_body(self):
        ...
        with patch("backend.services.feishu_service.httpx.AsyncClient", return_value=上下文):
    ...
    async def test_获取tenant_access_token_解析并缓存(self):
        ...
        with patch("backend.services.feishu_service.httpx.AsyncClient", return_value=上下文), \
                patch("backend.services.feishu_service.time.time", side_effect=[1000.0, 1001.0]):
    ...
    async def test_测试webhook_成功与失败(self):
        ...
        with patch("backend.services.feishu_service.httpx.AsyncClient", return_value=成功上下文):
            服务 = 飞书服务(webhook_url="https://open.feishu.cn/hook/test")
        ...
        with patch("backend.services.feishu_service.httpx.AsyncClient", return_value=失败上下文):
            服务 = 飞书服务(webhook_url="https://open.feishu.cn/hook/test")
...
class 测试_飞书接口:
    def test_test_webhook_成功返回统一响应(self):
        ...
        with patch("backend.api.feishu_api.飞书服务.测试webhook", new=AsyncMock(return_value=True)):
