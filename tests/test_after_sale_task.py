...
class 测试_售后任务:
    ...
    def test_注册名为_售后处理(self):
        ...
        import tasks.after_sale_task as 售后任务模块
    ...
    def test_判断售后类型_标准化归类(self, 原始类型: str, 期望类型: str):
        from tasks.after_sale_task import 售后任务
    ...
    async def test_执行_API多页扫描并写入队列(self, 模拟页面):
        ...
        with patch("tasks.after_sale_task.上报", new_callable=AsyncMock), patch(
                "tasks.after_sale_task.售后页",
                return_value=模拟售后页,
        ):
            from tasks.after_sale_task import 售后任务
    ...
    async def test_执行_重复页全部已扫描时停止翻页(self, 模拟页面):
        ...
        with patch("tasks.after_sale_task.上报", new_callable=AsyncMock) as 模拟上报, patch(
            "tasks.after_sale_task.售后页",
            return_value=模拟售后页,
        ):
            from tasks.after_sale_task import 售后任务
