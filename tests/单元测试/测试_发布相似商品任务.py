...
class 测试_发布相似商品任务:
    ...
    def 模拟发布页(self):
        ...
        发布页对象.从URL提取新商品ID = MagicMock(return_value="init-1001")
        ...
        发布页对象.从成功页提取商品ID = MagicMock(return_value="new-2002")
    ...
    async def test_缺少_parent_product_id_抛ValueError(self, 模拟回调, 模拟页面):
        ...
        with patch("tasks.发布相似商品任务.上报", new_callable=AsyncMock), \
            ...
            with pytest.raises(ValueError, match="parent_product_id"):
                await 任务.执行(
                    模拟页面,
    ...
    @pytest.mark.asyncio
    @patch("browser.任务回调._回调", new_callable=AsyncMock)
    async def test_正常流程_返回成功且包含_new_product_id(
        self,
        模拟回调,
        模拟页面,
        模拟商品列表页,
        ...
        assert 结果 == "成功"
        assert 任务._执行结果["new_product_id"] == "new-2002"
        assert 任务._执行结果["parent_product_id"] == "1001"
        模拟商品列表页.搜索商品.assert_awaited_once_with("1001")
        模拟发布页.修改标题.assert_awaited_once_with("新的标题")
        模拟发布页.关闭页面.assert_awaited_once()
        assert 模拟回填.await_count == 2

    @pytest.mark.asyncio
    @patch("browser.任务回调._回调", new_callable=AsyncMock)
    async def test_new_title为空时不调用_修改标题(
        self,
        模拟回调,
        模拟页面,
        模拟商品列表页,
