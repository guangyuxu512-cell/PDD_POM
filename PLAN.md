...
## Prompt 49：任务注册表 Schema 扩展与通用任务接口 ✅

- [x] 扩展 `tasks/注册表.py`，为 `register_task(...)` 增加 `input_schema`、`output_schema`、`category`、`tags`、`timeout`、`retry_policy`
- [x] 支持在声明 `input_schema` 且未显式传入 `required_fields` 时自动推导必填字段
- [x] 更新 `获取所有任务()` / `获取任务元数据()`，返回任务分类、标签、Schema、超时和重试策略等元数据
- [x] 更新 `tasks/任务注册表.py` 兼容导出层，补齐新的注册表能力导出
- [x] 新增 `backend/api/通用任务接口.py`
- [x] 新增任务注册表接口：
  - `GET /api/task-registry/`
  - `GET /api/task-registry/{task_name}/schema`
  - `POST /api/task-registry/{task_name}/validate`
- [x] 更新 `backend/api/路由注册.py`，注册通用任务接口路由
- [x] 更新 `backend/services/执行服务.py`，流程预检接入 `input_schema` 自动校验
- [x] 更新 `tasks/售后任务.py`，补充 `售后任务输入` Schema，并声明分类/标签/超时
- [x] 新增 `docs/task-plugin.md`，沉淀新增任务的标准模板
- [x] 更新 `tests/单元测试/测试_任务注册表.py`
- [x] 新增 `tests/单元测试/测试_任务注册表扩展.py`
- [x] 更新 `tests/单元测试/测试_执行服务.py`
- [x] 定向验证通过：
  - `python -m pytest -c tests/pytest.ini -q tests/单元测试/测试_任务注册表.py tests/单元测试/测试_任务注册表扩展.py tests/单元测试/测试_执行服务.py tests/单元测试/测试_店铺和流程接口.py`
- [x] 定向验证结果：`25 passed`
- [x] 全量验证通过：`python -m pytest -c tests/pytest.ini -q`
- [x] 全量验证结果：`446 passed, 16 warnings`

## Prompt 50：Electron 桌面壳与 Windows 打包 ✅

- [x] 更新 `backend/启动入口.py`
- [x] 新增前端构建目录自动探测，支持源码模式与 PyInstaller 冻结后的目录结构
- [x] 在所有 API 路由注册后挂载 `frontend/dist` 静态资源
- [x] 为非 `/api`、`/docs`、`/redoc`、`/openapi.json` 路径增加 SPA 回退到 `index.html`
- [x] 新增 `electron/main.js`
- [x] 新增 `electron/preload.js`
- [x] 新增 `electron/package.json`
- [x] 新增 `build_backend.bat`
- [x] 新增 `build_all.bat`
- [x] 新增 `scripts/pyinstaller_entry.py`
- [x] 新增 `scripts/pyinstaller_celery_entry.py`
- [x] 更新 `tests/单元测试/测试_启动入口.py`
- [x] 新增回归覆盖：
  - 前端静态文件可被 FastAPI 正常返回
  - 非 API 路径会回退到 SPA 首页
  - 后端保留路径不会被错误回退到前端
- [x] 定向验证通过：
  - `python -m pytest -c tests/pytest.ini -q tests/单元测试/测试_启动入口.py`
- [x] 定向验证结果：`3 passed`
- [x] 全量验证通过：`python -m pytest -c tests/pytest.ini -q`
- [x] 全量验证结果：`447 passed, 16 warnings`
- [ ] 前端构建命令在当前执行环境验证未通过：
  - `npm --prefix frontend run build`
  - 失败原因：`esbuild` 在当前环境启动子进程时报 `spawn EPERM`

## Prompt 120：Electron 打包切到嵌入式 Python 源码方案 ✅

- [x] 新增 `scripts/prepare_dist.py`
- [x] 按任务要求复制 `backend/`、`tasks/`、`browser/`、`pages/`、`selectors/`、`scripts/`、`data/`、`.env`、`requirements.txt` 到 `dist_bundle/`
- [x] `scripts/prepare_dist.py` 支持跳过 `__pycache__/`、`*.pyc`、`.git/`、`node_modules/`、`frontend/`
- [x] `scripts/prepare_dist.py` 支持下载并解压 Python 3.12 嵌入版、改写 `python312._pth`
- [x] 新增 `scripts/build_all.bat`，串联 `prepare_dist`、前端构建和 Electron 打包
- [x] 更新 `electron/main.js`
- [x] 生产模式改为从 `resources/app/python-embed/python.exe` 直接启动 `uvicorn` 与 `celery`
- [x] 删除旧的 `python-backend/*.exe` 启动路径、`packagedBackendExe`、`packagedCeleryExe`、`ensurePackagedFileExists()`
- [x] 更新 `electron/package.json`
- [x] `extraResources` 改为打入 `../dist_bundle` 和 `../frontend/dist`
- [x] 更新 `.gitignore`，补充 `dist_bundle/`、`python-backend-dist/`、`electron/dist/`、`*.spec`、`build/`
- [x] 新增 `tests/单元测试/测试_prepare_dist.py`
- [x] 更新 `tests/单元测试/测试_生产环境审查.py`
- [x] 定向验证通过：
  - `python -m pytest -c tests/pytest.ini -q tests/单元测试/测试_prepare_dist.py`
  - `python -m pytest -c tests/pytest.ini -q tests/单元测试/测试_生产环境审查.py -k Electron生产打包`
- [x] 全量验证通过：`python -m pytest -c tests/pytest.ini -q`
- [x] 全量验证结果：`451 passed, 16 warnings`
