...
from backend.models.scheduled_task_model import 标准化店铺ID列表
from backend.services.shop_service import 店铺服务实例
from backend.services.flow_service import 流程服务实例
from backend.services.execute_service import 执行服务实例, 获取队列名称
...
class 定时执行服务:
    ...
    async def _移除RedBeat计划(self, 计划ID: str) -> None:
        ...
        try:
            条目 = RedBeatSchedulerEntry.from_key(计划键, app=celery_app)
        except KeyError:
            return
