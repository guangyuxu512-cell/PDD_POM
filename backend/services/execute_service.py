...
from backend.services.execute_flow_precheck_service import 执行流程预检服务
from backend.services import execute_state_store as 执行状态仓储
...
from tasks.registry import 获取任务元数据, 获取任务类, 初始化任务注册表
...
_流程预检服务工具 = 执行流程预检服务(
    允许空运行策略集合=允许空运行策略集合,
    任务参数字段别名映射=任务参数字段别名映射,
    获取任务元数据函数=lambda 名称: 获取任务元数据(名称),
)
...
def 同步读取批次状态(batch_id: str) -> Optional[Dict[str, Any]]:
    ...
    return 执行状态仓储.同步读取批次状态(
        batch_id,
        获取同步Redis客户端=同步获取Redis客户端,
        批次状态键=批次状态键,
        读取内存批次状态=_读取内存批次状态,
        记录Redis降级=_记录Redis降级,
    )


def 同步写入批次状态(批次数据: Dict[str, Any]) -> Dict[str, Any]:
    """供 Celery Worker 写入批次状态并推送事件。"""
    return 执行状态仓储.同步写入批次状态(
        批次数据,
        获取同步Redis客户端=同步获取Redis客户端,
        批次状态键=批次状态键,
        当前批次键=当前批次键,
        ...
        记录Redis降级=_记录Redis降级,
        写入内存批次状态=_写入内存批次状态,
        同步写入运行实例状态=同步写入运行实例状态,
        尝试发送批次完成回调=尝试发送批次完成回调,
    )
...
async def 设置取消标记(batch_id: str) -> bool:
    ...
    async def _关闭客户端(客户端) -> None:
        await 客户端.aclose()

    return await 执行状态仓储.设置取消标记(
        batch_id,
        批次取消键=批次取消键,
        批次取消标记过期秒=批次取消标记过期秒,
        内存取消标记缓存=_内存取消标记缓存,
...
class 执行服务:
    ...
    def _校验步骤输入(
        ...
        return self._流程预检服务.校验步骤输入(task_name, 上下文数据)
    ...
    async def _获取流程输入行映射(
        ...
        *,
        ...
        """读取输入集，并按店铺聚合启用输入行。"""
        return await self._流程预检服务.获取流程输入行映射(
            flow_id=flow_id,
    ...
    async def _创建输入集兼容流程参数(
        ...
        *,
        ...
        return await self._流程预检服务.创建输入集兼容流程参数(
            flow_id=flow_id,
            输入行映射=输入行映射,
        )
    ...
    async def _清理店铺残留流程参数记录(
        ...
        *,
        ...
        return await self._流程预检服务.清理店铺残留流程参数记录(
            flow_id=flow_id,
            shop_ids=shop_ids,
            流程参数记录映射=流程参数记录映射,
            记录跳过日志=lambda 店铺ID, 保留记录ID, 跳过记录ID列表: self._记录残留流程参数清理日志(
                flow_id,
                店铺ID,
                保留记录ID,
                跳过记录ID列表,
    ...
    def _记录残留流程参数清理日志(
        ...
        logger.info(
            ...
            f"keep_id={保留记录ID}, skipped_ids={跳过记录ID列表}"
        )
    ...
    def _构建运行项上下文映射(
        ...
        *,
        ...
        """为运行快照生成每个店铺的初始上下文。"""
        return self._流程预检服务.构建运行项上下文映射(
            flow_id=flow_id,
            shop_ids=shop_ids,
            流程参数记录映射=流程参数记录映射,
            输入行映射=输入行映射,
    ...
    async def _获取批次状态(self, batch_id: str) -> Optional[Dict[str, Any]]:
        """读取单个批次状态。"""
        return await 执行状态仓储.获取批次状态(
            batch_id,
            获取异步Redis客户端=self._获取异步Redis客户端,
            关闭异步Redis客户端=self._关闭异步Redis客户端,
            批次状态键=批次状态键,
            读取内存批次状态=_读取内存批次状态,
            记录Redis降级=_记录Redis降级,
        )
    ...
    async def 获取最新批次状态(self, batch_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        return await 执行状态仓储.获取最新批次状态(
            ...
            批次状态键=批次状态键,
            读取内存批次状态=_读取内存批次状态,
            获取内存当前批次ID=lambda: _内存当前批次ID,
            记录Redis降级=_记录Redis降级,
        )

    async def _写入批次状态(self, 批次数据: Dict[str, Any]) -> Dict[str, Any]:
        """异步写入批次状态并推送事件。"""
        return await 执行状态仓储.写入批次状态(
            批次数据,
            获取异步Redis客户端=self._获取异步Redis客户端,
            关闭异步Redis客户端=self._关闭异步Redis客户端,
            批次状态键=批次状态键,
    ...
    async def 投递单步任务(
        ...
        *,
        ...
        """创建并投递单个 Celery 步骤任务。"""
        from tasks.execute_task import 执行任务 as 执行Celery任务
    ...
    async def 停止批次(self, batch_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        for 任务ID in 批次数据.get("task_ids", []):
            if 任务ID:
                celery_app.control.revoke(任务ID, terminate=False)
