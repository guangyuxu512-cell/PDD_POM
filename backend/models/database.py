...
from backend.models.after_sale_config_model import 初始化售后配置表, 售后配置建表SQL
...
流程输入集建表SQL = """
    CREATE TABLE IF NOT EXISTS flow_input_sets (
        id TEXT PRIMARY KEY,
        flow_id TEXT NOT NULL,
        name TEXT NOT NULL,
        description TEXT,
...
流程输入行建表SQL = """
    CREATE TABLE IF NOT EXISTS flow_input_rows (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
        updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (input_set_id) REFERENCES flow_input_sets(id) ON DELETE CASCADE,
        FOREIGN KEY (shop_id) REFERENCES shops(id) ON DELETE CASCADE
    )
"""

流程输入集流程索引SQL = """
    CREATE INDEX IF NOT EXISTS idx_flow_input_sets_flow_id
    ON flow_input_sets (flow_id)
"""

流程输入行输入集店铺索引SQL = """
    CREATE INDEX IF NOT EXISTS idx_flow_input_rows_set_shop
    ON flow_input_rows (input_set_id, shop_id)
"""

流程输入行幂等键索引SQL = """
    CREATE INDEX IF NOT EXISTS idx_flow_input_rows_source_key
    ON flow_input_rows (source_key)
"""

运行实例建表SQL = """
    CREATE TABLE IF NOT EXISTS execution_runs (
        id TEXT PRIMARY KEY,
        mode TEXT NOT NULL CHECK(mode IN ('flow', 'task')),
        flow_id TEXT,
        task_name TEXT,
        ...
        actual_concurrency INTEGER NOT NULL DEFAULT 1,
        callback_url TEXT,
...
运行项建表SQL = """
    CREATE TABLE IF NOT EXISTS execution_run_items (
        ...
        status TEXT NOT NULL DEFAULT 'queued' CHECK(status IN ('queued', 'precheck_failed', 'waiting_barrier', 'running', 'success', 'failed', 'skipped', 'cancelled')),
        last_task_name TEXT,
        error TEXT,
...
运行步骤建表SQL = """
    CREATE TABLE IF NOT EXISTS execution_run_steps (
        ...
        task_name TEXT NOT NULL,
        on_fail TEXT NOT NULL DEFAULT 'abort',
        barrier INTEGER NOT NULL DEFAULT 0,
        merge INTEGER NOT NULL DEFAULT 0,
        status TEXT NOT NULL DEFAULT 'queued' CHECK(status IN ('queued', 'waiting_barrier', 'running', 'success', 'failed', 'skipped', 'cancelled')),
        ...
        finished_at DATETIME,
        updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (run_item_id) REFERENCES execution_run_items(id) ON DELETE CASCADE
    )
"""
...
运行实例流程索引SQL = """
    CREATE INDEX IF NOT EXISTS idx_execution_runs_flow_id
    ON execution_runs (flow_id)
"""
...
运行项批次索引SQL = """
    ...
    ON execution_run_items (run_id)
"""

运行项店铺索引SQL = """
    CREATE INDEX IF NOT EXISTS idx_execution_run_items_shop
    ON execution_run_items (run_id, shop_id)
"""

运行项状态索引SQL = """
    CREATE INDEX IF NOT EXISTS idx_execution_run_items_status
    ON execution_run_items (run_id, status)
"""

运行步骤运行项索引SQL = """
    CREATE INDEX IF NOT EXISTS idx_execution_run_steps_item
    ON execution_run_steps (run_item_id, step_index)
"""

运行步骤Celery索引SQL = """
    CREATE INDEX IF NOT EXISTS idx_execution_run_steps_celery
    ON execution_run_steps (celery_task_id)
"""
...
async def 初始化数据库() -> None:
    ...
    async with aiosqlite.connect(数据库路径, timeout=数据库忙等待毫秒 / 1000) as 连接:
        ...
        from backend.models.rule_model import 初始化规则表
    ...
    try:
        from backend.services.rule_service import 规则服务实例
